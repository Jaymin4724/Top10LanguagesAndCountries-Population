// Function to update content of a content div
function updateContent(contentDiv, title, bar, count) {
  ...
  barContainer.querySelector(".bar").style.height = "10px";
  ...
  fetch("https://restcountries.com/v3.1/all?fields=name,population")
    ...
    .then((data) => {
      ...
      top10Countries.forEach((country, index) => {
        ...
        const bar = `${
          (country.population / top10Countries[0].population) * 100
        }%`;
...
// Function to display the top 10 languages by usage in countries// Function to display the top 10 languages by usage in countries
